# Fix my_team.html template - available_pokemon is list of tuples (pokemon, shiny)
            ...
            <a href="{% url 'pokemon:pokemon_detail' member.pokemon.id %}" class="team-member-link">
                <div class="team-member-card rarity-{{ member.pokemon.rarity|lower }}">
                    <div class="member-icon">
                        🐾{% if team_shiny|get_item:member.id %}<span class="shiny-star">⭐</span>{% endif %}
                    </div>
                    <h3>{{ member.nickname|default:member.pokemon.name }}{% if team_shiny|get_item:member.id %}<span class="shiny-star">⭐</span>{% endif %}</h3>
            ...
            <div class="pokemon-grid">
                ...
                <a href="{% url 'pokemon:pokemon_detail' p.0.id %}" class="team-member-link">
                    <div class="pokemon-card">
                        <div class="member-icon" style="font-size: 2.5rem;">🐾{% if p.1 %}<span class="shiny-star">⭐</span>{% endif %}</div>
                        <h4>{{ p.0.name }}{% if p.1 %}<span class="shiny-star">⭐</span>{% endif %}</h4>
